...
| 类型 | 公式 | 遍历顺序 |
| :--------------: | :------------------------------------------------------------------ | :-------------------- |
| 01 背包 | | 外`物品`**升**序 内`背包`**降**序 |
| 完全背包 | | 外`物品`**升**序 内`背包`**升**序 |
| 组合问题（不强调顺序） | `dp[i] += dp[i - num]` | 外`物品`**升**序 内`背包`**升**序 |
| 排列问题 (强调顺序) | `dp[i] += dp[i - num]` |外`背包`**升**序 内`物品`**升**序 |
| True、False 问题 | `dp[i] = dp[i] or dp[i - num]` | |
| 最大最小问题 | `dp[i] = min(dp[i], dp[i - num] + 1) or dp[i] = max(dp[i], dp[i - num] + 1)` | |
